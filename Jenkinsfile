pipeline {
    ...
    environment {
        IMAGE_NAME = "yash5090/ballsbouncing"
	...
	stage('Docker Authentication with ECR') {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-Bncing-Bll-Deployment.git'
            }
        }
	...
	stage('Publishing Docker Image to Container Registry') {
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop ballsbouncing | true"
                sh "docker rm ballsbouncing | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh "docker run -d --name ballsbouncing -p 5000:80 ${IMAGE_NAME}:${TAG}" 
                //}
            }
        }
        ...
        stage('Switch Traffic Between Blue & Green Environment') {
            ...
            steps {
                script {
                    ...
                    withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'k8s', namespace: '', restrictKubeConfigAccess: false, serverUrl: '') {
                        sh '''
                            kubectl patch service ballsbouncing-service -p "{\\"spec\\": {\\"selector\\": {\\"app\\": \\"ballsbouncing\\", \\"version\\": \\"''' + newEnv + '''\\"}}}"
                        '''
                    }
